...
public class part2 {
	...
	// here, efficiency is taken into consideration. If the file, is valid, it won't be accessed again.
	/**
	 * 
	...
	public static void main(String[] args) {
		...
		for(int i = 1; i <= 10; i++) {
			...
			try{
				if(processFilesForValidation(filename, i)) {
					createIEEE(filename, i);
					createACN(filename, i);
				}
			}catch(Exception e) {
				System.out.println("Could not open Latex"+ i +".bib Please check if file exists! Program will terminate after closing any opened files.");
				pivot = i;
				badFile = true;
				break;
			}			
